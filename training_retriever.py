...
def valid(dataset, llm, retriever, tensorizer, config, task, epoch):
    ...
    for i, ind in enumerate(idx_list):
        ...
        with torch.no_grad():
            ...
            prefer_loss,pred = llm.inference(query_entry,selected_ctxs,answer_list,label,force_pred=True)
            all_loss+=[prefer_loss.item()] if isinstance(prefer_loss,torch.Tensor) else []
            if llm.option_num>1:
...
def train(train_dataset, llm, retriever, tensorizer, optimizer, scheduler, scaler, prompt_parser, config, epoch):
    ...
    train_loss = 0.0
    total_ctrs_loss,total_prefer_loss,total_ortho_loss = 0.0,0.0,0.0
    ...
    for i, ind in enumerate(idx_list):
        ...
        # if  config.ctrs_loss_penalty>0 and len(data.positive_idx_list)>0:
        ...
        #     loss = loss + config.ctrs_loss_penalty*ctrs_loss

        # caculate ortho_loss
        if config.ortho_loss_penalty>0:
            ortho_loss = retriever.calculate_ortho_loss(similarity)
            total_ortho_loss += ortho_loss.item()
            loss = loss + config.ortho_loss_penalty*ortho_loss

        # get k-shot mask
        prefer_loss = llm.get_llm_loss(similarity,query_entry,ctx_entries,answer_list,label,epoch)
        total_prefer_loss+=prefer_loss.item()
        loss = loss + config.preference_penalty*prefer_loss
        ...
        if (i+1)%config.batch_size==0:
            ...
            scheduler.step()
            config.update_cnt+=1
            info_text = f"CTRS: {total_ctrs_loss:.1f}, PREFER: {total_prefer_loss:.1f}, ORTHO: {total_ortho_loss:.1f}"
...
def parse_args():
    ...
    parser.add_argument("--preference_penalty", type=float, default=0.001, help="")
    # parser.add_argument("--ctrs_loss_penalty", type=float, default=0, help="")
