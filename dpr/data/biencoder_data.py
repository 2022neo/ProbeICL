...
class ProbeIclDataset(Dataset):
    ...
    def get_example_answers(self, entry):
        task = task_map.cls_dic[entry["task_name"]]()
    ...
    def get_entry(self, entry):
        if self.loss_type == 'dpr':
            ...
            question = ""
            if self.use_choosen==2:
                for id in entry["choosen"]:
            ...
            answers,label = self.get_example_answers(entry)
            entry = edict({
                "query_ctx": query_ctx,
                "answers": answers,
                "label":label,
    ...
    def load_data(self):
        ...
        if self.loss_type == 'dpr':
            if self.use_choosen>0:
                self.data = [r for r in self.data if len(r["positive_ctxs"]) > 0]
            else:
                self.data = [r for r in self.data if len(r["positive_ctxs"]) > 0 and len(r["choosen"])==0 and len(r["hard_negative_ctxs"]) > 0]
