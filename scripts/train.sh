...
for ortho_loss_penalty in 1; do
    for ctrs_loss_penalty in 1; do
        for label_loss_penalty in 0.001; do
            for neg_step in 1 -1; do
                for pos_step in 1; do
                    ...
                    --dropout 0.1 --mask_type 3 --top_k 80  --pos_step $pos_step --neg_step $neg_step 
                    # inference
                    sh scripts/inference.sh $gpu_ids $exps_dir $task_name
                    ...
                    --ortho_loss_penalty $ortho_loss_penalty --ctrs_loss_penalty $ctrs_loss_penalty --label_loss_penalty $label_loss_penalty\
                    --temperature 1 --multi_ctrs 1 --hard_mask 1 --rand_neg 0 \
                    --dropout 0.2 --mask_type 3 --top_k 60  --pos_step $pos_step --neg_step $neg_step 
                    # inference
                    sh scripts/inference.sh $gpu_ids $exps_dir $task_name
                    ...
                    --ortho_loss_penalty $ortho_loss_penalty --ctrs_loss_penalty $ctrs_loss_penalty --label_loss_penalty $label_loss_penalty \
                    --temperature 1 --multi_ctrs 1 --hard_mask 1 --rand_neg 0 \
                    --dropout 0.3 --mask_type 3 --top_k 80  --pos_step $pos_step --neg_step $neg_step 
                    # inference
                    sh scripts/inference.sh $gpu_ids $exps_dir $task_name
                done
            done
...
for ortho_loss_penalty in 1; do
    for ctrs_loss_penalty in 0.0001; do
        for label_loss_penalty in 1; do
            for neg_step in 1 -1; do
                for pos_step in 1; do
                    CUDA_VISIBLE_DEVICES=$gpu_ids python training_retriever.py \
                    --exps_dir $exps_dir --task_name $task_name --epoches 10 --learning_rate 0.00001 \
